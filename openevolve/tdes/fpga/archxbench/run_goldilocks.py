"""
Goldilocks experiment: does fine-grained test granularity unlock TDES crossover
on Level-3 FP designs?

Hypothesis: the coarse suite (3 VerilogTests) gives zero gradient to TDES —
every gen-1 mutation scores 0 tests, no complementary coverage, stagnation.
The fine-grained suite (24 VerilogTests) gives sub-case resolution so that a
candidate correctly handling NaN-×-zero has a *different* pass-set from one
correctly handling Inf-×-normal, enabling complementary-coverage crossover.

Designs:
  fp_multiplier       — coarse (3 tests): baseline / reference
  fp_multiplier_fine  — fine  (24 tests): hypothesis test

Conditions:
  single_agent_30    — iterative CEGIS baseline (no population)
  tdes_full          — full TDES (crossover + memory)
  tdes_no_crossover  — TDES without crossover (ablation)

Headline metric: did crossover fire (crossover_attempts > 0 in tdes_full)?
Secondary: does tdes_full outperform tdes_no_crossover on fine tests?

Usage:
    python -m openevolve.tdes.fpga.archxbench.run_goldilocks \\
        --config openevolve/tdes/fpga/experiments/configs/anthropic_opus.yaml \\
        --seeds 0 1 2 --out tdes_fpga_goldilocks_results

    # Offline validation (scripted mutations):
    python -m openevolve.tdes.fpga.archxbench.run_goldilocks --scripted --seeds 0
"""
...
ALL_DESIGNS    = ["fp_multiplier", "fp_multiplier_fine"]
...
_DEFAULT_OUT = "tdes_fpga_goldilocks_results"
...
def _render_table(results: List[metrics.RunMetrics], designs: List[str]) -> str:
    lines = [
        "# TDES-FPGA Goldilocks Experiment",
        "",
        "**Hypothesis**: fine-grained tests (24 sub-case VerilogTests) unlock TDES crossover",
        "on Level-3 FP multiplier by giving partial implementations non-zero gradient.",
        "",
        "## Solve Rate",
        "",
    ]
    ...
    for d in sorted(designs):
        for cond in ALL_CONDITIONS:
            ...
            calls = []
            for s in [0, 1, 2]:
                m = next((x for x in results
                          if x.design == d and x.condition == cond and x.seed == s), None)
                if m:
            ...
            if any(c != "—" for c in row):
                lines.append(
                    f"| {d} | {cond} | {row[0]} | {row[1]} | {row[2]} | {med} |"
                )
...
def main(argv=None):
    setup_logging()
    p = argparse.ArgumentParser(description="TDES Goldilocks experiment")
    ...
    metrics_path = os.path.join(args.out, "metrics_goldilocks.json")
